const input = `febca cfagb ecbafd efdcbg cbegdfa fg bgafec gfae acgdb gfc | cgf facdeb ecgfdb afcbge
...
const countUniqueOutoutDigits = (data) => {
  const parsedByLine = data.split(`\n`)
  const parsedOutputs = parsedByLine.map((line) => line.split(' | ')[1])
  const parsedOutPutDigits = parsedOutputs.map(output => output.split(' '))
  let count = 0
  for (const output of parsedOutPutDigits) {
...
// console.log(countUniqueOutoutDigits(input));

const deciferInputDigits  = (line) => {
  const parsedInputs = line.split(' | ')[0]
  const parsedInputDigits = parsedInputs.split(' ')
  const map = {}

  const findOne = parsedInputDigits.filter(digit => digit.length === 2)[0]
  map[`${findOne}`] = '1'
  const findFour = parsedInputDigits.filter(digit => digit.length === 4)[0]
  map[`${findFour}`] = '4'
  const findSeven = parsedInputDigits.filter(digit => digit.length === 3)[0]
  map[`${findSeven}`] = '7'
  const findEight = parsedInputDigits.filter(digit => digit.length === 7)[0]
  map[`${findEight}`] = '8'

  const findSix = parsedInputDigits.filter(digit => digit.length === 6 && findOne.split('').some(character => !digit.split('').includes(character)))[0]
  map[`${findSix}`] = '6'
  const findZero = parsedInputDigits.filter(digit => digit !== findSix && digit.length === 6 && findFour.split('').some(character => !digit.split('').includes(character)))[0]
  map[`${findZero}`] = '0'
  const findNine = parsedInputDigits.filter(digit => digit !== findSix && digit !== findZero && digit.length === 6)[0]
  map[`${findNine}`] = '9'

  const findThree = parsedInputDigits.filter(digit => digit.length === 5 && findSeven.split('').every(character => digit.split('').includes(character)))[0]
  map[`${findThree}`] = '3'
  const findFive = parsedInputDigits.filter(digit => digit !== findThree && digit.length === 5 && digit.split('').includes(findSix.split('').filter(character => findOne.includes(character))[0]))[0]
  map[`${findFive}`] = '5'
  const findTwo = parsedInputDigits.filter(digit => digit !== findThree && digit !== findFive && digit.length === 5)[0]
  map[`${findTwo}`] = '2'

  return map
}

const deciferOutput = (data) => {
  const map = deciferInputDigits(data)

  const output = data.split(' | ')[1]
  const outputDigits = output.split(' ')
  const digitsDecifered = outputDigits.map(digit => {
    for (const key in map) {
      if (key.length !== digit.length) continue
      if (key.split('').every(character => digit.split('').includes(character))) {
        return map[key]
      }
    }
  })

  return parseInt(digitsDecifered.join(''))
}

const sumOutputs = (data) => {
  let count = 0;
  for (const line of data.split(`\n`)) {
    count += deciferOutput(line)
  }
  return count
}
